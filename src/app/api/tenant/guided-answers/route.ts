...
import { QUESTION_REGISTRY, isQuestionApplicable, nextQuestion, nextQuestionForStages, publicQuestion, validateQuestionAnswer } from "@/lib/guided-question-registry";
...
export async function POST(request: Request) {
  try {
    ...
    const answerId = guidedAnswerDocumentId(session.id, input.subjectKey, input.questionId, input.candidateId); if (!answerId) throw new GuidedAccessError("Answer identity is invalid.", 400);
    ...
    await adminDb.runTransaction(async (transaction) => {
      ...
      if (input.subjectType === "session" && input.subjectKey !== currentSession.id || input.subjectType === "candidate" && input.subjectKey !== candidate?.id || input.subjectType === "step" && input.stepKey !== input.subjectKey) throw new GuidedAccessError("Answer subject is invalid.", 409);
      ...
      const answerLimit = candidate ? MAX_GUIDED_ANSWERS_PER_CANDIDATE : 100;
      ...
      if (question.stage === "procedure_outputs" || question.stage === "confirmation") {
        ...
        const basicsPending = nextQuestionForStages(canonicalAnswers, candidate!.id, "candidate", ["procedure_basics", "procedure_inputs"]);
      ...
      if (existing && (existing.sessionId !== currentSession.id || existing.candidateId !== input.candidateId)) throw new GuidedAccessError("Answer identity collision.", 409);
      ...
      for (const questionId of obsoleteQuestions) { const obsoleteAnswerId = guidedAnswerDocumentId(currentSession.id, input.subjectKey, questionId, input.candidateId); if (obsoleteAnswerId) obsoleteSnapshots.push(await transaction.get(adminDb.collection("clarificationItems").where("answerId", "==", obsoleteAnswerId).limit(2))); }
      ...
      const answerData = { companyId: currentSession.companyId, projectId: currentSession.projectId, organizationUnitId: currentSession.organizationUnitId, sessionId: currentSession.id, subjectType: input.subjectType, subjectKey: input.subjectKey, questionId: input.questionId, ruleId: question.ruleId, answer: input.answer, certainty: input.certainty, active: true, answeredBy: actorPath, ...(input.candidateId ? { candidateId: input.candidateId } : {}), ...(input.stepKey ? { stepKey: input.stepKey } : {}), ...(input.uncertaintyNote ? { uncertaintyNote: input.uncertaintyNote } : {}), updatedAt: now };
      ...
      for (const snapshot of obsoleteSnapshots) for (const document of snapshot.docs) { const item = parsePersistedClarification(document.id, document.data()); if (item && item.sessionId === currentSession.id && item.candidateId === input.candidateId && item.status === "open") transaction.update(document.ref, { status: "resolved", active: false, resolutionAnswerId: answerId, resolutionNote: "لم يعد السؤال منطبقًا بعد تحديث الإجابة المرتبطة.", resolvedAt: now, updatedBy: actorPath, updatedAt: now }); }
